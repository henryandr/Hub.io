...
class BlueGigaAPI(object):
    ...
    def close_serial(self): # Added to close serial serial port
        self._serial.close()
    ...
    def parse_bgapi_packet(self, packet, callbacks=None):
        ...
        rx_payload = packet[4:]
        # if(packet_class != 6):
        #     print("0x%02X 0x%02X 0x%02X" % (message_type,packet_class,packet_command))
        if technology_type == 0x1: # 0x0 is BLE, 0x1 is Wi-fi
    ...
    def parse_bgapi_event(self, packet_class, packet_command, rx_payload, callbacks=None):
        ...
        if packet_class == 0:
            ...
            if packet_command == 0:
                ...
                connection, atthandle, atttype, value_len = struct.unpack('<BHBB', rx_payload[:5])
                callbacks.ble_evt_attclient_attribute_value(connection=connection, atthandle=atthandle, atttype=atttype, value=rx_payload[5:])
            elif packet_command == 6:
